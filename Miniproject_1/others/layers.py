...
# CNR block: Convolution + Normalization + ReLU
class CNR(nn.Module):
...
# DNR block: Deconvolution + Normalization + ReLU
class DNR(nn.Module):
    def __init__(self, nch_in, nch_out, kernel_size=4, stride=1, padding=1, output_padding=0, norm='bnorm', relu=0.0, drop=[], bias=[]):
        ...
        self.dblock = nn.Sequential(*layers)
...
# Padding method, default set to zero padding
class Padding(nn.Module):
    def __init__(self, padding, padding_mode='zero', value=0):
...
# default polling set to average pooling
...
# upsampling with nearest neighbor method
class UpSampling(nn.Module):
    def __init__(self, nch=[], pool=2, type='nearest'):
        ...
        if type == 'nearest':
            ...
            self.upsampling = nn.Upsample(scale_factor=pool, mode='bilinear', align_corners=True)
        elif type == 'conv':
            self.upsampling = nn.ConvTranspose2d(nch, nch, kernel_size=pool, stride=pool)
